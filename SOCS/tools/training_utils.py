...
def get_gt_v(Rs, axis=2):
    ...
    if axis == 3:
        ...
        corners = corners.view(1, 3, 3).repeat(bs, 1, 1)  # bs x 3 x 3
        gt_vec = torch.bmm(Rs, corners).transpose(2, 1).reshape(bs, -1)
    gt_green = gt_vec[:, 3:6] # second row is y
    gt_red = gt_vec[:, (6, 7, 8)] # third row is x
    return gt_green, gt_red

def get_gt_v_modelnet(Rs):
    bs = Rs.shape[0]  # bs x 3 x 3
    corners = torch.tensor([[0, 0, 1], [0, 0,0], [0, 1, 0]], dtype=torch.float).to(Rs.device)
    corners = corners.view(1, 3, 3).repeat(bs, 1, 1)  # bs x 3 x 3
    gt_vec = torch.bmm(Rs, corners).transpose(2, 1).reshape(bs, -1)

    gt_green = gt_vec[:, 3:6] # first row is z
    gt_red = gt_vec[:, (6, 7, 8)] # third row is x
    return gt_green, gt_red
