...
class GCN3D_segR(nn.Module):
    ...
    def forward(self,vertices: "tensor (bs, vetice_num, 3)"):
        ...
        feature_dict_detach={'conv0':{'pos':vertices,'fea':fm_0.detach()},
                      'conv1':{'pos':vertices,'fea':fm_1.detach()},
                      'conv2':{'pos':v_pool_1,'fea':fm_2.detach()},
                      'conv3':{'pos':v_pool_1,'fea':fm_3.detach()},
                      'conv4':{'pos':v_pool_2,'fea':fm_4.detach()},
        ...
        point_fea=fm_fuse
        global_fea=f_global
        return recon,point_fea,global_fea,feature_dict,feature_dict_detach
...
class MyQNet(nn.Module):
    ...
    def forward(self, query_points, feature_dicts,stage='first',debug=False):
        ...
        pre_bin_dict={
            'first':self.fc_out_first(query_features),
            'second':self.fc_out_second(query_features)
        }
        return pre_bin_dict
...
class Decoder(nn.Module):
    def __init__(self, in_ch,out_ch,feat_ch,use_tanh=False,dropout=False,dropout_prob=0.2):
