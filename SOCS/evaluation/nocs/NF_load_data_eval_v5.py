...
from evaluation.utils.eval_utils_v1 import get_3d_bbox, transform_coordinates_3d, compute_3d_iou_new
import json
from nfmodel.uti_tool import *
...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='test',
        ...
        models = {}
        model_name2cat_name,models=get_nocs_models(self.data_dir)
        name_path=os.path.join(self.data_dir,'obj_models/name2cat.txt')
        if not os.path.exists(name_path):
            with open(name_path,'w') as f:
                json.dump(model_name2cat_name,f)
        else:
            with open(name_path,'r') as f:
                model_name2cat_name=json.load(f)
        self.models=models
        self.model_name2cat_name=model_name2cat_name
        ...
        grids['fake_grid']=make_grid(FLAGS.fake_grid_num,uniform_scale)
        ...
        self.re_transformer={}
        self.cat2std={}
        for cat_came in self.cat_names:
            cur_cat_models=[]
            for k,v in models.items():
                if model_name2cat_name[k]==cat_came:
                    cur_cat_models.append(k)
            cur_cat_models=cur_cat_models
            std_model_name=cat_came+'mean'
            # std_model=models[std_model_name]
            if cat_came=='camera':
                std_model=models[cur_cat_models[10]]
                # show_open3d(std_model,std_model)
            else:
                std_model=models[std_model_name]
            self.cat2std[cat_came]=std_model
            ...
            for model_name in cur_cat_models:
                ...
                if FLAGS.align_origin==1:
                    pad1=np.zeros((1,3)).astype(np.float32)
                    pad2=np.zeros((1,3)).astype(np.float32)
                    std_keypoint_pad=np.concatenate([pad1,std_keypoint],axis=0)
                    cur_keypoint_pad=np.concatenate([pad2,cur_keypoint],axis=0)
                    if FLAGS.regular_tps:
                        re_tps=TPS_reguler(std_keypoint_pad,cur_keypoint_pad)
                        tps=TPS_reguler(cur_keypoint_pad,std_keypoint_pad)
                    else:
                        re_tps=TPS(std_keypoint_pad,cur_keypoint_pad)
                        tps=TPS(cur_keypoint_pad,std_keypoint_pad)
                else:
                    if FLAGS.regular_tps:
                        re_tps=TPS_reguler(std_keypoint,cur_keypoint)
                        tps=TPS_reguler(cur_keypoint,std_keypoint)
                    else:
                        re_tps=TPS(std_keypoint,cur_keypoint)
                        tps=TPS(cur_keypoint,std_keypoint)
                self.transformer[model_name]=tps
                self.re_transformer[model_name]=re_tps
        if FLAGS.shuffle:
    ...
    def __getitem__(self, index):
        ...
        std_models=[]
        model_points=[]
        match=self.match_box(detection_dict['pred_bboxes'],detection_dict['gt_bboxes'])


        gt_num_instance=len(detection_dict['gt_class_ids'])
        detection_dict['model']=['' for _ in range(gt_num_instance)]
        detection_dict['gt_nocs_scale']=[-1 for _ in range(gt_num_instance)]
        for i in range(gt_num_instance):
            cat_id =detection_dict['gt_class_ids'][i]
            cur_bbox=detection_dict['gt_bboxes'][i]
            cur_match=self.match_box(cur_bbox[None,:],gts['bboxes'])
            if cat_id==6:
                detection_dict['gt_RTs'][i][:3,:3]=gts['rotations'][cur_match[0]]
                detection_dict['gt_RTs'][i][:3,3]=gts['translations'][cur_match[0]]
                model = self.models[gts['model_list'][cur_match[0]]].copy().astype(np.float32) # 1024 points
                nocs_scale = gts['scales'][cur_match[0]]
                ...
                ly_t = ly * nocs_scale
                lz_t = lz * nocs_scale
                detection_dict['gt_scales'][i]=np.array([lx_t, ly_t, lz_t])
            detection_dict['model'][i]=gts['model_list'][cur_match[0]]
            detection_dict['gt_nocs_scale'][i]=gts['scales'][cur_match[0]]
        for j in range(num_instance):
            ...
            model_name=detection_dict['model'][match_index]
            std_model=self.cat2std[self.cat_names[cat_id-1]]
            try:
                tps=self.transformer[model_name]
            except:
                print(model_name,'not in cur_cat_models')
                tps=self.transformer[self.cat_names[cat_id-1]+'mean']

            gt_nocs_scale=detection_dict['gt_nocs_scale'][match_index]
            model_point=self.models[model_name]*gt_nocs_scale
            # model_indices = np.random.randint(len(model_point), size=1024)
            # model_point=model_point[model_indices]
            # tmp_std_model=self.std_model.copy()
            # std_min_point=np.min(self.std_model ,axis=0,)
            # std_max_point=np.max(self.std_model ,axis=0,)
            # std_size=std_max_point-std_min_point
            # min_point=np.min(model ,axis=0,)
            # max_point=np.max(model ,axis=0,)
            # size=max_point-min_point
            # deform_std_model=self.re_cur_cat_transform[model_idx](self.std_model/std_size)*size
            ...
            gt_R=detection_dict['gt_RTs'][match_index, :3, :3]
            factor=np.cbrt(np.linalg.det(gt_R[:3, :3]))
            gt_R=gt_R/factor
            gt_scale=gt_scale*factor
            gt_T=detection_dict['gt_RTs'][match_index, :3, 3]

            std_models.append(std_model)
            model_points.append(model_point)
            coefficients.append(tps.coefficient)
            control_points.append(tps.control_points)
            gt_Rs.append(gt_R)
            gt_Ts.append(gt_T)
        ...
        data_dict['std_models'] = torch.as_tensor(std_models.astype(np.float32)).repeat(roi_imgs.shape[0],1,1).contiguous()
        data_dict['model_points'] = torch.as_tensor(model_points.astype(np.float32)).contiguous()
        ...
        data_dict['gt_Rs'] = torch.as_tensor(gt_Rs, dtype=torch.float32).contiguous()
        data_dict['gt_Ts'] = torch.as_tensor(gt_Ts, dtype=torch.float32).contiguous()
        data_dict['gt_scales'] = torch.as_tensor(gt_scales, dtype=torch.float32).contiguous()
        data_dict['coefficients'] = torch.as_tensor(coefficients, dtype=torch.float32).contiguous()
        data_dict['control_points'] = torch.as_tensor(control_points, dtype=torch.float32).contiguous()
    ...
    def get_mean_shape(self, c):
        # if  (self.source=='Real' and FLAGS.transfer!=1) or self.source=='CAMERA+Real':
    ...
    def match_box(self,bboxes1,bboxes2):
        # yx yx
        def iou(bbox1,bbox2):
            overlap_min_y=max(bbox1[0],bbox2[0])
            overlap_min_x=max(bbox1[1],bbox2[1])
            overlap_max_y=min(bbox1[2],bbox2[2])
            overlap_max_x=min(bbox1[3],bbox2[3])
            if overlap_min_x>=overlap_max_x or overlap_min_y>=overlap_max_y:
                return 0
            inter=(overlap_max_x-overlap_min_x)*(overlap_max_y-overlap_min_y)
            union=(bbox1[3]-bbox1[1])*(bbox1[2]-bbox1[0])+(bbox2[3]-bbox2[1])*(bbox2[2]-bbox2[0])-inter
            return inter/union
        match=[]
        for i,bbox1 in enumerate(bboxes1):
            ...
            for j,bbox2 in enumerate(bboxes2):
                cur_iou=iou(bbox1,bbox2)
                if cur_iou>iou_max:
                    iou_max=cur_iou
                    match_index=j
            match.append(match_index)
        return match
