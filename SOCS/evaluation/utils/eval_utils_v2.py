...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, class_name_1, class_name_2):
    ...
    symmetry_flag = False
    if (class_name_1 in ['nut', 'screw'] and class_name_1 == class_name_2):
        # print('*'*10)
...
def compute_RT_degree_cm_symmetry(RT_1, RT_2, class_id, synset_names):
    ...
    if synset_names[class_id] in ['screw']:
        y = np.array([0, 1, 0])
        y1 = R1 @ y
        y2 = R2 @ y
        theta = np.arccos(
    ...
    elif synset_names[class_id] in ['hnm']:
        y_180_RT = np.diag([-1.0, 1.0, -1.0])
        R = R1 @ R2.transpose()
        R_rot = R1 @ y_180_RT @ R2.transpose()
        theta = min(np.arccos((np.trace(R) - 1) / 2),
                    np.arccos((np.trace(R_rot) - 1) / 2))
    elif synset_names[class_id] in ['nut']:
        sym_xzs=[]
        sym_xzs.append(np.array([[1, 0, 0], [0, 1, 0],[0, 0, 1]]))
        sym_xzs.append(np.array([[1, 0, 0], [0, -1, 0],[0, 0, -1]]))
        n=6
        sym_ys=[]
        for i in range(n):
            t=float(i/n)*2*math.pi
            sym_ys.append(np.array([[math.cos(t), 0, math.sin(t)],
                                    [0, 1, 0],
                                    [-math.sin(t), 0, math.cos(t)]]))
        thetas=[]
        for sym_xz in sym_xzs:
            for sym_y in sym_ys:
                R_rot=R1 @ sym_y @ sym_xz @ R2.transpose()
                can_theta=np.arccos((np.trace(R_rot) - 1) / 2)
                thetas.append(can_theta)
        theta=min(thetas)
