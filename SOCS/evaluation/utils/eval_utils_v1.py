...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, handle_visibility, class_name_1, class_name_2):
    ...
    def asymmetric_3d_iou(RT_1, RT_2, scales_1, scales_2):
        ...
        # show_open3d(bbox_3d_2.T,bbox_3d_1.T)
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=True):
    ...
    fig_iou = plt.figure(figsize=(12,4))
    ...
    plt.grid()
    plt.xlabel('3D IoU thresholds',fontsize=15)


    iou_dict = {}
    iou_dict['thres_list'] = iou_thres_list
    iou_thres_np=np.array(iou_thres_list)*100
    for cls_id in range(1, num_classes):
        ...
        ax_iou.plot(iou_thres_np, iou_3d_aps[cls_id, :]*100, label=class_name)

    iou_3d_aps[-1, :] = np.mean(iou_3d_aps[1:-1, :], axis=0)
    ax_iou.plot(iou_thres_np, iou_3d_aps[-1, :]*100,  label='mean')
    ...
    for cls_id in range(1, num_classes):
        ...
        # print(class_name)
        ax_trans.plot(
            degree_thres_list[:-1], pose_aps[cls_id, :-1, -1]*100, label=class_name)

    ax_trans.plot(degree_thres_list[:-1], pose_aps[-1, :-1, -1]*100, label='mean')
    pose_dict['aps'] = pose_aps

    ax_rot = plt.subplot(133)
    plt.ylim((0, 1))
    plt.xlabel('translation/cm',fontsize=15)
    plt.ylim((0, 100))
    plt.xlim((0, 10))
    ticks=[0,5,10]
    ax_rot.set_xticks(ticks)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
    plt.grid()
    for cls_id in range(1, num_classes):
        ...
        ax_rot.plot(shift_thres_list[:-1],
                    pose_aps[cls_id, -1, :-1]*100, label=class_name)

    ax_rot.plot(shift_thres_list[:-1], pose_aps[-1, -1, :-1]*100, label='mean')
