...
class PoseDataset(data.Dataset):
    def __init__(self, source=None, mode='train', data_dir=None,
        ...
        self.composed_dir=os.path.join(self.data_dir,'composed_depth')
        ...
        cur_cat_transform=[]
        re_cur_cat_transform=[]
        std_model_name=per_obj+'mean'
        ...
        for model_name in cur_cat_models:
            ...
            re_cur_cat_transform.append(re_tps)
        self.cur_cat_transform=cur_cat_transform
        self.re_cur_cat_transform=re_cur_cat_transform
    ...
    def __getitem__(self, index):
        ...
        if 'CAMERA' in img_path.split('/'):
            ...
            try:
                if FLAGS.aug_bg:
                    path_split=self.img_list[index].split('/')[1:]
                    comp_depth_path=os.path.join(self.composed_dir,*path_split)+'_composed.png'
                    comp_depth=load_depth(comp_depth_path)
            except:
                comp_depth=None
        else:
        ...
        if 'CAMERA' in img_path.split('/') and FLAGS.aug_bg and comp_depth is not None:
            comp_depth[mask_target==1.0]=depth[mask_target==1.0]
            depth=comp_depth
        ...
        tps=self.cur_cat_transform[model_idx]
        deform_sdf_points=self.re_cur_cat_transform[model_idx](sdf_points)
        all_index=np.arange(len(self.cur_cat_models))
        target_idx=int(np.random.choice(all_index,1))
        re_tps=self.re_cur_cat_transform[target_idx]
        ...
        data_dict['std_model']= torch.as_tensor(self.std_model,dtype=torch.float32).contiguous()
        ...
        data_dict['tps_coefficient']=torch.as_tensor(tps.coefficient,dtype=torch.float32).contiguous()
        data_dict['tps_control_points']=torch.as_tensor(tps.control_points,dtype=torch.float32).contiguous()
        data_dict['re_tps_coefficient']=torch.as_tensor(re_tps.coefficient,dtype=torch.float32).contiguous()
        data_dict['re_tps_control_points']=torch.as_tensor(re_tps.control_points,dtype=torch.float32).contiguous()
        return data_dict
