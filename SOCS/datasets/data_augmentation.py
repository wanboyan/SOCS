...
# point cloud based data augmentation
# augment based on bounding box
def defor_3D_bb(pc, R, t, s, sym=None, aug_bb=None,model_points=None,nocs_scale=None):
    # pc  n x 3, here s must  be the original s
    pc_reproj = torch.mm(R.T, (pc - t.view(1, 3)).T).T  # nn x 3
    if sym[0] == 1:  # y axis symmetry
        ...
        if model_points is not None:
            model_points[:,(0, 2)]=model_points[:,(0, 2)]* exz
            model_points[:,1]=model_points[:,1]*ey
        s[0] = s[0] * exz
        s[1] = s[1] * ey
        s[2] = s[2] * exz
        pc_new = torch.mm(R, pc_reproj.T) + t.view(3, 1)
        ...
        # show_open3d(pc_reproj.cpu().numpy(),(nocs_scale*model_points+0.1).cpu().numpy())
        if model_points is not None:
            return pc_new, s,model_points
        else:
            return pc_new, s
    else:
        ex = aug_bb[0]
        ...
        pc_reproj[:, 2] = pc_reproj[:, 2] * ez
        if model_points is not None:
            model_points[:,0]=model_points[:,0]*ex
            model_points[:,1]=model_points[:,1]*ey
            model_points[:,2]=model_points[:,2]*ez
        s[0] = s[0] * ex
        s[1] = s[1] * ey
        s[2] = s[2] * ez
        pc_new = torch.mm(R, pc_reproj.T) + t.view(3, 1)
        pc_new = pc_new.T
        # show_open3d(pc_reproj.cpu().numpy(),(nocs_scale*model_points).cpu().numpy())
        if model_points is not None:
            return pc_new, s,model_points
        else:
            return pc_new, s
...
def defor_3D_bc(pc, R, t, s, model_point, nocs_scale,base0_id=None):
    ...
    pc_new = torch.mm(R, pc_reproj.T) + t.view(3, 1)
    pc_new = pc_new.T
    model_point_t=model_point*nocs_scale
    model_point_resize = (model_point_t[:, 1] + s[1] / 2) / s[1] * (ey_up - ey_down) + ey_down
    model_point_t[:, 0] = model_point_t[:, 0] * model_point_resize
    model_point_t[:, 2] = model_point_t[:, 2] * model_point_resize
    if base0_id==5:
        lx = 2 * torch.max(torch.abs(model_point_t[:, 0]))
        ly = 2 * torch.max(torch.abs(model_point_t[:, 1]))
        lz = 2 * torch.max(torch.abs(model_point_t[:, 2]))
    else:
        lx = max(model_point_t[:, 0]) - min(model_point_t[:, 0])
        ly = max(model_point_t[:, 1]) - min(model_point_t[:, 1])
        lz = max(model_point_t[:, 2]) - min(model_point_t[:, 2])
    # show_open3d(pc_reproj.cpu().numpy(),(model_point_t).cpu().numpy())
    return pc_new, torch.tensor([lx, ly, lz], device=pc.device)
